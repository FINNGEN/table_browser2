...
@app.route('/api/v1/variant_annotation/<variant>')
def variant_annotation(variant):
    try:
        var = Variant(variant)
        anno = fetch.get_anno(var)
    except ParseException as e:
        return jsonify({'message': repr(e)}), 400
    except NotFoundException as e:
        return jsonify({'message': repr(e)}), 404
    return jsonify(anno)
...
# use run.py instead of running directly, this is here for debug purposes only
