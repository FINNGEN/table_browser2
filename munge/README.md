...
```
gsutil cat gs://fg-cromwell_fresh/scrape_annots/8bcb6383-66af-4575-b12e-72955211a8f5/call-small/annotated_variants.small.gz | zcat | \
awk -f filter_annotation_to_coding.awk | \
awk '
# reindex from 0
BEGIN{OFS="\t"}
NR==1 { for(i=1;i<=NF;i++) h[$i]=i; print $0 }
NR>1  { $h["index"]=NR-2; print $0 }
' | bgzip -@2 > r9_imp_anno_coding_INFO0.6.gz
```
...
```
import pandas as pd
imp = pd.read_csv('r9_imp_anno_coding_INFO0.6.gz', sep='\t')
chip = pd.read_csv('r9_chip_anno_coding.gz', sep='\t')
...
anno.to_csv('r9_imp_chip_anno.tsv', sep='\t', na_rep='NA', index_label='#index', columns=['variant', 'chr', 'pos', 'ref', 'alt', 'INFO', 'AF', 'AC_Het', 'AC_Hom', 'AF_chip', 'AC_Het_chip', 'AC_Hom_chip', 'most_severe', 'gene_most_severe', 'rsid', 'enrichment'])
```

tabix:

```
bgzip r9_imp_chip_anno.tsv && tabix -s 3 -b 4 -e 4 r9_imp_chip_anno.tsv.gz
```
...
Compute snpstats with a GP threshold of 0.9 to get high-probability homozygotes as per imputation:

TODO Sanni how is `R9_coding_variants_INFO0.6.bgen` created

```
qctool -g R9_coding_variants_INFO0.6.bgen -threshold 0.9 -snp-stats -osnp R9_coding_variants_INFO0.6.snpstats_gp0.9
...
import pandas as pd
import scipy.stats
anno = pd.read_csv('r9_imp_chip_anno.tsv.gz', sep='\t')
ss = pd.read_csv('R9_coding_variants_INFO0.6.snpstats_gp0.9', sep='\t')
...
proflevel.to_csv('r9_imp_chip_anno_proflevel.tsv', sep='\t', na_rep='NA', index=False, columns=[
'variant',
'chr',
'pos',
'ref',
...
```
cut -f1 r9_imp_chip_anno_proflevel.tsv | tail -n+2 > proflevel_vars
awk 'NR==FNR{a[$1]=1} NR>FNR&&(FNR==1||($1 in a))' proflevel_vars <(zcat /mnt/nfs/annotations/r9/R9_annotated_variants_vgnomadfixed.gz) | bgzip > proflevel_vars_anno.gz
...
import pandas
d = pandas.read_csv("r9_imp_chip_anno_proflevel.tsv", sep="\t")
n = pandas.read_csv("proflevel_vars_nchip", sep="\t")
d.merge(n, how="left", on="variant").to_csv("r9_imp_chip_anno_proflevel_nchip.tsv", sep="\t", index=False)
```
